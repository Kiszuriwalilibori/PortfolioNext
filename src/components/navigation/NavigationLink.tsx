...
function NavigationLink({ page, clickHandler, currentPathName }: Props) {
    ...
    return (
        <ListItem id="List Item" disablePadding sx={navigationItemSx}>
            <Button component={Link} scroll={false} href={PageUtils.pageToHref(page)} rel="noopener" onClick={clickHandler} sx={navigationButtonSx(isActive)} aria-current={isActive ? "page" : undefined} disableRipple>
